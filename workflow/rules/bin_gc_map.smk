...
rule compute_mappability:
    ...
    shell: # Assumes all short read files have same length of reads
        ...
        if [[ "{input.random_read_file}" == *.gz ]]; then
            ...
            mkdir "results/genmap/tmp_{wildcards.progenitor}" 2> "{log}"
        ...
        bash {workflow.basedir}/scripts/map_in_bins.sh {params.bin_size} "results/genmap/tmp_{wildcards.progenitor}.genmap.intersect.bed" "results/genmap/tmp_{wildcards.progenitor}.genmap.unsorted.bed" mkdir -p $(dirname {output.genmap_index_dir}) 2> "{log}"
