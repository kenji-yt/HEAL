...
rule compute_mappability:
    ...
    shell: # Assumes all short read files have same length of reads
        ...
        if [[ "{input.random_read_file}" == *.gz ]]; then
            ...
            mkdir -p "results/genmap/tmp_{wildcards.progenitor}" 2> "{log}"
            gunzip -c "{input.random_read_file}" > "$read_file" 2>> "{log}"
        else
        ...
        read_length=$(
            {
                head -1000 "$read_file" | \
                seqtk seq -A | \
                awk '{{ if (NR % 2 == 0) print length($0) }}' | \
                sort | uniq -c | sort -nr | head -1 | \
                awk '{{ print $2 }}'
            } 2>> "{log}"
        )

        mkdir -p "$(dirname "{output.genmap_index_dir}")" 2>> "{log}"
        genmap index -F "{input.assembly}" -I "{output.genmap_index_dir}" 2>> "{log}"
        genmap map -K "$read_length" -E 0 -I "{output.genmap_index_dir}" -O "{output.genmap_index_dir}" -bg 2>> "{log}"
        ...
        bedtools intersect -a "{input.bin_bed}" -b "$genmap_bedgraph" -wo > "results/genmap/tmp_{wildcards.progenitor}.genmap.intersect.bed" 2>> "{log}"

        bash {workflow.basedir}/scripts/map_in_bins.sh {params.bin_size} \
            "results/genmap/tmp_{wildcards.progenitor}.genmap.intersect.bed" \
            "results/genmap/tmp_{wildcards.progenitor}.genmap.unsorted.bed"

        bedtools sort -i "results/genmap/tmp_{wildcards.progenitor}.genmap.unsorted.bed" > "$mappa_bed" 2>> "{log}"

        rm -r results/genmap/tmp_{wildcards.progenitor}* 2>> "{log}"
        """